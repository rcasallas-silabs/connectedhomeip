...
sl_status_t ExampleApp::Toggle(LedId led)
{
    switch(led)
    {
    case LedId::Led0:
        sl_si91x_led_toggle((uint8_t)led_led0.pin);
        break;
    case LedId::Led1:
        sl_si91x_led_toggle((uint8_t)led_led1.pin);
        break;
    }
    return SL_STATUS_OK;
}
...
void sl_si91x_button_isr(uint8_t pin, int8_t state)
{
    ...
    bool pressed = (0 == state);

    if (!pressed) return;
    if (pin == button_btn0.pin) {
        app.OnEvent(silabs::EventId::OnPress0, (void*)pressed);
    }
    else if (pin == button_btn1.pin) {
        app.OnEvent(silabs::EventId::OnPress1, (void*)pressed);
    }
}