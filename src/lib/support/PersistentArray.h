...
static constexpr size_t kPersistentArraySerializedMinSize = 7 + 200; // ContainerTag(1) + CountTag(1) + Count(2) + ListTag(1) + EndListTag(1) + EndContainerTag(1) + 1
...
struct PersistentArray : public PersistentData<kPersistentArraySerializedMinSize + kMaxListSize * (kPersistentEntrySerializedMinSize + kMaxSerializedEntrySize)>
{
    PersistentArray(PersistentStorageDelegate * storage, uint16_t count = 0) :
        PersistentData<kPersistentArraySerializedMinSize + kMaxListSize * (kPersistentEntrySerializedMinSize + kMaxSerializedEntrySize)>(storage), mLimit(kMaxListSize),
        mCount(count)
    {}
...
protected:
    ...
    virtual CHIP_ERROR Copy(EntryType & dest, const EntryType & src) const = 0;
    // {
    //     dest = src;
    //     return CHIP_NO_ERROR;
    // }
