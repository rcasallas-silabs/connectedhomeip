...
public:
    ...
    static StorageKeyName FabricGroupKey(chip::FabricIndex fabric, uint32_t index)
    {
        return StorageKeyName::Formatted("f/%x/gk/%lx", fabric, index);
    }
    ...
    {
        return StorageKeyName::Formatted("f/%x/k/%lx", fabric, keyset);
    }
