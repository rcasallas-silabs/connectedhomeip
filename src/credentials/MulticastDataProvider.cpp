...
static constexpr TLV::Tag KeyListTag() { return TLV::ContextTag(5); }
static constexpr TLV::Tag EncryptKeyTag() { return TLV::ContextTag(6); }
static constexpr TLV::Tag PrivacyKeyTag() { return TLV::ContextTag(7); }
static constexpr TLV::Tag ExpirationTag() { return TLV::ContextTag(8); }
...
struct TargetList: public PersistentArray<CHIP_CONFIG_MAX_FABRICS, kPersistentBufferMax, TargetEntry>
{
    TargetList(FabricIndex fabric, PersistentStorageDelegate * storage):
        PersistentArray<CHIP_CONFIG_MAX_FABRICS, kPersistentBufferMax, TargetEntry>(storage),
        mFabric(fabric) {}
    ...
    CHIP_ERROR Deserialize(TLV::TLVReader & reader, TargetEntry & entry) override
    {
        ...
        VerifyOrReturnError(TLV::kTLVType_Array == reader.GetType(), CHIP_ERROR_INTERNAL);
        {
            ...
            for(size_t i = 0; i < kTargetKeyCount; ++i)
            {
                ...
                memcpy(entry.keys[i].encryption.AsMutable<Crypto::Symmetric128BitsKeyByteArray>(), buf.data(), sizeof(Crypto::Symmetric128BitsKeyByteArray));
                // Privacy Key
                ReturnErrorOnFailure(reader.Next(PrivacyKeyTag()));
                ReturnErrorOnFailure(reader.Get(buf));
                memcpy(entry.keys[i].privacy.AsMutable<Crypto::Symmetric128BitsKeyByteArray>(), buf.data(), sizeof(Crypto::Symmetric128BitsKeyByteArray));
                // Endpoint Count
...
DataProvider::KeyContext::KeyContext(DataProvider &provider, const TargetKey &key) :
    mProvider(provider)
{
    memcpy(mEncryptionKey.AsMutable<Crypto::Symmetric128BitsKeyByteArray>(), key.encryption.As<Crypto::Symmetric128BitsKeyByteArray>(), sizeof(Crypto::Symmetric128BitsKeyByteArray));
    memcpy(mPrivacyKey.AsMutable<Crypto::Symmetric128BitsKeyByteArray>(), key.privacy.As<Crypto::Symmetric128BitsKeyByteArray>(), sizeof(Crypto::Symmetric128BitsKeyByteArray));
}
...
size_t DataProvider::KeyContextIterator::Count()
{
    ...
    for(size_t i = 0; i < kTargetKeyCount; ++i)
    {
        if(mEntry.keys[0].IsActive()) { count++; }
    }
    return count;
}
...
DataProvider::TargetIterator::TargetIterator(DataProvider & provider, FabricIndex fabric):
    mProvider(provider), mFabric(fabric)
{
    chip::Multicast::TargetList list(fabric, provider.mStorage);
    list.Load();
    mCount = list.Count();
...
CHIP_ERROR DataProvider::SetTarget(const FabricInfo *fabric, Target &target, const ByteSpan &key, uint32_t period)
{
    ...
    // Insert entry
    {
        ...
        else
        {
            // Set new key
            TargetKey key;
            ReturnErrorOnFailure(mKeystore->CreateKey(credentials.encryption_key, key.encryption));
            ReturnErrorOnFailure(mKeystore->CreateKey(credentials.privacy_key, key.privacy));
            entry.SetActiveKey(key, period);
        }
