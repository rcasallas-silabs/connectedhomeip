...
struct TargetList : public PersistentArray<CHIP_CONFIG_MAX_FABRICS, kPersistentBufferMax, GroupEntry>
{
    ...
    CHIP_ERROR UpdateKey(StorageKeyName & key) const override
    {
        ...
        key = DefaultStorageKeyAllocator::MulticastGroups(mFabric);
        return CHIP_NO_ERROR;
    }
...
CHIP_ERROR DataProvider::SetGroup(const FabricInfo * fabric, Group & target, const ByteSpan & key, uint32_t period)
{
