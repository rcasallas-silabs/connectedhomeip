...
struct Tags
{
    static constexpr TLV::Tag Policy() { return TLV::ContextTag(1); }
    static constexpr TLV::Tag NumKeys() { return TLV::ContextTag(2); }
    static constexpr TLV::Tag GroupCredentials() { return TLV::ContextTag(3); }
    static constexpr TLV::Tag StartTime() { return TLV::ContextTag(4); }
    static constexpr TLV::Tag KeyHash() { return TLV::ContextTag(5); }
    static constexpr TLV::Tag KeyValue() { return TLV::ContextTag(6); }
...
CHIP_ERROR KeysetList::Serialize(TLV::TLVWriter & writer, const KeysetData & entry) const
{
    ...
    // operational_keys
    {
        ...
        ReturnErrorOnFailure(writer.StartContainer(Tags::GroupCredentials(), TLV::kTLVType_Array, array));
        uint8_t keyCount   = 0;
        ...
        for (auto & key : entry.operational_keys)
        {
            ...
            ReturnErrorOnFailure(writer.Put(Tags::StartTime(), static_cast<uint64_t>(startTime)));
            ReturnErrorOnFailure(writer.Put(Tags::KeyHash(), hash));
            ReturnErrorOnFailure(writer.Put(Tags::KeyValue(), ByteSpan(encryptionKey)));
...
CHIP_ERROR KeysetList::Deserialize(TLV::TLVReader & reader, KeysetData & entry)
{
    ...
    {
        // operational_keys
        ReturnErrorOnFailure(reader.Next(Tags::GroupCredentials()));
