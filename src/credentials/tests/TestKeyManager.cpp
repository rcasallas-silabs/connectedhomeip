...
GroupDataProviderImpl TestGroupDataProvider::sManager;
...
TEST_F(TestGroupDataProvider, TestIpk)
{
    ...
    EXPECT_EQ(chip::Credentials::SetSingleIpkEpochKey(keys, kFabric1, ByteSpan(kIpkEpochKeyFromSpec), kCompressedFabricId1),
              CHIP_NO_ERROR);
    EXPECT_EQ(keys->GetIpkKeySet(kFabric1, ipkOperationalKeySet), CHIP_NO_ERROR);
...
{
    GroupDataProvider * keys = GetGroupDataProvider();
    EXPECT_TRUE(keys);
...
TEST_F(TestGroupDataProvider, TestGroupDecryption)
{
    ...
    // Get the key context
    Crypto::SymmetricKeyContext * key_context = keys->GetKeyContext(kFabric2, kGroup2);
