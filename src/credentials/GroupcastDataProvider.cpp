...
struct Tags {
    ...
    static constexpr TLV::Tag EndpointCount() { return TLV::ContextTag(2); }
    static constexpr TLV::Tag EndpointList() { return TLV::ContextTag(3); }
    static constexpr TLV::Tag EndpointEntry() { return TLV::ContextTag(31); }
...
DataProvider::KeyIterator::KeyIterator(DataProvider & provider, FabricTable *fabrics, GroupId group_id, uint16_t session_id) :
    mProvider(provider), mFabrics(fabrics), mGroupId(group_id), mContext(provider), mSessionId(session_id)
{
}
...
bool DataProvider::KeyIterator::Next(KeyContext *&output)
{
    ...
    while (mFabrics && (mFabricIndex < mFabrics->FabricCount()))
    {
        ...
        if(info)
        {
            ...
            {
                // GroupId find in fabric, iterate keys
                while(mKeyIndex < kGroupKeyCount)
                {
                    auto &key = entry.keys[mKeyIndex++];
                    if (key.IsActive() && (key.hash == mSessionId))
                    {
                        mContext.Set(key);
                        output = &mContext;
...
CHIP_ERROR DataProvider::SetEndpoints(FabricIndex fabric_idx, Group & group)
{
    chip::Groupcast::GroupList list(fabric_idx, mStorage);
    GroupEntry entry(group.group_id);
    size_t index = 0;
    // Find target in NVM
    ReturnErrorOnFailure(list.Find(entry, index));
    entry.endpoint_count = group.endpoint_count;
...
{
    chip::Groupcast::GroupList list(fabric_idx, mStorage);
    ReturnErrorOnFailure(list.Remove(GroupEntry(group_id)));
    return list.Save();
}
