...
CHIP_ERROR GroupcastLogic::JoinGroup(FabricIndex fabric_index, const Groupcast::Commands::JoinGroup::DecodableType & data)
{
    ...
    // Add group
    ReturnErrorOnFailure(provider.SetGroup(fabric->GetFabricIndex(), group));

    ReturnErrorOnFailure(RegisterAccessControl(fabric->GetFabricIndex(), group.groupID));
...
CHIP_ERROR GroupcastLogic::RegisterAccessControl(FabricIndex fabric_index, GroupId groupID)
{
    return CHIP_ERROR_NOT_IMPLEMENTED;
}
