...
using GroupData = chip::Groupcast::Data;
                                          ...
                                          AttributeValueEncoder & aEncoder)
{
    ...
    CHIP_ERROR err = aEncoder.EncodeList([&groups, fabric_index](const auto & encoder) -> CHIP_ERROR {
        CHIP_ERROR status = CHIP_NO_ERROR;
        auto result       = groups->IterateGroups(fabric_index, [&](auto & iter) -> CHIP_ERROR {
            // Group
            GroupData entry;
...
CHIP_ERROR GroupcastLogic::JoinGroup(FabricIndex fabric_index, const Groupcast::Commands::JoinGroup::DecodableType & data)
{
    ...
    // Key handing
    group.keySetID = static_cast<KeysetId>(data.keySetID);
    ...
    // Add group
    ReturnErrorOnFailure(groups->SetGroup(fabric_index, group));
    // Join multicast
                                      ...
                                      Groupcast::Commands::LeaveGroupResponse::Type & response)
{
    ...
    ReturnErrorOnFailure(groups->GetGroup(fabric_index, old_data));
    if (data.endpoints.HasValue())
    {
        ...
        {
            return groups->SetEndpoints(fabric_index, new_data);
        }
    }

    // Remove whole group
    response.endpoints = DataModel::List<const chip::EndpointId>(old_data.endpoints, old_data.endpointCount);
    return groups->RemoveGroup(fabric_index, data.groupID);
}
