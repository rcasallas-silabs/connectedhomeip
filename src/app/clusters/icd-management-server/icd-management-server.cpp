    ...
    return encoder.EncodeList([supported_clients](const auto & subEncoder) -> CHIP_ERROR {
        ...
        for (const auto & fabricInfo : fabricTable)
        {
            ...
            for (uint16_t i = 0; i < table.Limit(); ++i)
            {
                ...
                Structs::MonitoringRegistrationStruct::Type s {
                    .checkInNodeID = e.checkInNodeID,
...
{
    return encoder.Encode(GetIcdManagementServer().GetICDCounter());
}

CHIP_ERROR IcdManagementAttributeAccess::ReadClientsSupportedPerFabric(EndpointId endpoint, AttributeValueEncoder & encoder)
{
    return encoder.Encode(GetIcdManagementServer().GetClientsSupportedPerFabric());
}
                                                       ...
                                                       const Commands::RegisterClient::DecodableType & commandData)
{
    ...
    if (CHIP_NO_ERROR == CheckAdmin(commandObj, commandPath, is_admin))
    {
        ...
        status = GetIcdManagementServer().RegisterClient(storage, fabric, commandData.checkInNodeID, commandData.monitoredSubject,
                                                         commandData.key, commandData.verificationKey, is_admin);
    }
                                                         ...
                                                         const Commands::UnregisterClient::DecodableType & commandData)
{
    ...
    if (CHIP_NO_ERROR == CheckAdmin(commandObj, commandPath, is_admin))
    {
        PersistentStorageDelegate & storage = chip::Server::GetInstance().GetPersistentStorage();
        FabricIndex fabric                  = commandObj->GetAccessingFabricIndex();
        status = GetIcdManagementServer().UnregisterClient(storage, fabric, commandData.checkInNodeID, commandData.key, is_admin);
    }
...
{
    InteractionModel::Status status = GetIcdManagementServer().StayActiveRequest(commandObj->GetAccessingFabricIndex());
