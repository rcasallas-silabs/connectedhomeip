...
struct PersistentList: public PersistentData<kMaxSerializedSize> {
    ...
    PersistentList(PersistentStorageDelegate * storage): PersistentData<kMaxSerializedSize>(), mStorage(storage) {}
...
struct PersistentArray: public PersistentList<kMaxSerializedSize, kMaxListSize, EntryType> {
    ...
    EntryType & At(uint16_t index) { return mEntries[index].value; }
