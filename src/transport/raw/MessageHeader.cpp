...
CHIP_ERROR PacketHeader::DecodeFixedCommon(Encoding::LittleEndian::Reader & reader)
{
    ...
    if(HasDestinationNodeId())
    {
        ...
        if (Header::SessionType::kGroupSession == GetSessionType())
        {
            mFabricIndex.SetValue(static_cast<uint8_t>(destination_id >> 16 & 0xff));
            mDestinationGroupId.SetValue(static_cast<uint16_t>(destination_id & 0xffff));
        }
    }
