...
This application demonstrates how to configure SiWx91x device as an HTTP client and do HTTP PUT, GET and POST operations with the HTTP server opened on remote peer. 
In this application, the SiWx91x is configured as a Wi-Fi station and connects to an Access point. The application then performs HTTP PUT, GET and POST operations with 
...
4. Now choose **Wi-Fi- NCP HTTP Client v6** example for NCP mode and click on **Create**.
...
1. Check for CHIP_9117 and RSI_CONFIGURE_IPV6 macro in preprocessor settings as mentioned below.
   ...
   - If CHIP_9117 and RSI_CONFIGURE_IPV6 macro is not present, add it by clicking on add macro option.
...
2. Navigate to **<Si917 COMBO SDK> → utilities → scripts**. Run **simple_http_server_v6.py** acript on port number 80 using the following command.
