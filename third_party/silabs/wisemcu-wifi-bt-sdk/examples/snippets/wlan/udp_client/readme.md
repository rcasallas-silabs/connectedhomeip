...
![Figure: Setup Diagram SoC Mode for UDP Client Example](resources/readme/udpclientsoc.png)
...
![Figure: Setup Diagram NCP Mode for UDP Client Example](resources/readme/udpclientncp.png)
...
4. Now choose Wi-Fi- NCP Access Point example for NCP mode or choose Wi-Fi- SoC Access Point example for SoC mode and click on **Create**.
   ...
   **![UDP client project](resources/readme/udp_client_example_ncp.png)**

   For SoC mode:
      
   **![UDP client project](resources/readme/udp_client_example_soc.png)**

5. Give the desired name to your project and cick on **Finish**.

   **![Create UDP client project](resources/readme/udp_client_create.png)**
...
1. Wi-Fi Configuration
  ...
  - Configure the following parameters in **rsi_udp_client.c** to enable your Silicon Labs Wi-Fi device to connect to your Wi-Fi network.
...
2. Client/Server IP Settings
    ...
    ```c
    #define DEVICE_PORT        <local port>     //UDP client port number
    #define SERVER_PORT        <remote port>    //Remote UDP server port number which is opened in Windows PC2.   
    #define SERVER_IP_ADDRESS   "192.168.10.100" //Remote server IP address. 
    #define NUMBER_OF_PACKETS   <no of packets>  //How many packets to receive from UDP client. (Ex: #define NUMBER_OF_PACKETS 10000)
    ```
...
4. Whether IP address is configured through DHCP or STATIC
...
2. Click on the build icon (hammer) or right click on project name and choose **Build Project** to build the project.
   ...
   ![Build Project for NCP mode](resources/readme/udp_client_build_project_ncp.png)
...
1. Once the build was successful, right click on project and select **Debug As → Silicon Labs ARM Program** to program the device as shown in below image.
   ...
   **![debug_mode_NCP](resources/readme/udp_client_program_mode.png)**

2. As soon as the debug process is completed, the application control branches to the main().

3. Click on the **Resume** icon in the Simplicity Studio IDE toolbar to run the application.

   **![Run](resources/readme/run_mode.png)**

4. Configure the Access point in OPEN / WPA-PSK/WPA2-PSK mode in order to connect Silicon Labs device in STA mode.

5. Open UDP server application using iperf application in Windows PC2 which is connected to Access point through LAN.

  `iperf_demo.exe –s -u -p <SERVER_PORT> -i 1`

   ![Command](resources/readme/udpport.png)

6. After program gets executed, The Silicon Labs device would scan and connect to Access point and get IP.

7. After successful connection, the device STA connects to UDP server socket opened on Windows PC2 using UDP client socket and sends configured NUMBER_OF_PACKETS to remote UDP server. Please refer the below image for reception of UDP data on UDP server.

   ![Command](resources/readme/udplog.png)
