...
int32_t application()
{
  ...
  rsi_inet_ntop6(&ip_buff_6[1], link_local_addr, sizeof(link_local_addr));
  rsi_inet_ntop6(&ip_buff_6[5], &global_addr, sizeof(global_addr));
  rsi_inet_ntop6(&ip_buff_6[9], &gateway6, sizeof(gateway6));
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nIP Config Failed, Error Code : 0x%lX\r\n", status);
    return status;
  } else {
