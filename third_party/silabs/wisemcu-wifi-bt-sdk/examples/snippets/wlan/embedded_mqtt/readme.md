...
![Figure: Setup Diagram for Embedded MQTT Example: SoC](resources/readme/embeddedmqttclientsetupsoc.png)
### **3.2 NCP** 

![Figure: Setup Diagram for Embedded MQTT Example: NCP](resources/readme/embeddedmqttclientsetupncp.png)
...
To create the Embedded MQTT example project in the Simplicity Studio IDE, follow the [Creation of Project]() section in the respective ***Getting started with SiWx91x*** guides. 
   - For SoC, choose the **Wi-Fi - SoC Embedded MQTT** example.
   - For NCP, choose the **Wi-Fi - NCP Embedded MQTT** example.


## **6 Application configuration**
Read through the following sections and make any changes needed. 
  
1. In the Project explorer pane of the IDE, expand the **embedded_mqtt** folder and open the **rsi_emb_mqtt.c** file. Configure the following parameters based on your requirements.

   ![Application configuration](resources/readme/embeddedmqttapplicationconfiguration.png)
...
2. MQTT client and server/broker IP and port settings
   ```c
...
1. Before running the application, run the MQTT broker/server on a remote PC. To run MQTT broker on port 1883 in Windows PC1
   - Starting with the release of Mosquitto version 2.0.0, the default config allows local connections only. Therefore, the following lines have to be added in the **mosquitto.conf** file. If an older version of mosquitto broker is used, the following lines need not be added in the **mosquitto.conf** file.  
   ```c
...
2. Set up the MQTT client (MQTT explorer) application on windows PC so as to see the messages published by SiWx91x module.
   ...
   - Delete the existing topic names if any. Enter the desired **TOPIC_NAME** in topic field and click on **ADD**. Then the desired topic name can be observed in the topic list and click on **BACK** as shown in below image.
      ...
      **![Add TOPIC NAME in topic field](resources/readme/mqttexploreraddtopic.png)**
...
3. Once the SiWx91x gets connected to the MQTT broker and subscribes to the topic **RSI_MQTT_TOPIC (Ex: "MQTT_DEMO")**. The user can see the client connection and subscription success information in the MQTT broker.
   ...
   **![Client connected and subscription success information](resources/readme/mosquittoconnectsubscribe.png)**
...
5. MQTT Explorer which is running on Windows PC2 will receive the message published by SiWx91x module as it subscribed to the same topic.
   ...
   **![MQTT Explorer receives the message published by the SiWx91x EVK](resources/readme/mqttexplorermsg.png)**

6. In the MQTT broker, user can observe the published message as the MQTT client is subscribed to that topic. 

   **![In MQTT broker, user can observe the published message](resources/readme/mosquittopublishfrommodule.png)**
...
8. Application prints can be seen as follows:

   - **SoC**
      
      ![Application prints](resources/readme/embeddedmqttclientoutputprintssoc.png)

   - **NCP**

      ![Application prints](resources/readme/embeddedmqttclientoutputprintsncp.png)
