...
//! HTTP/HTTPS include file to refer HTTPS APIs
...
//! Set HTTP_POST_DATA to use HTTP/HTTPS POST LARGE DATA feature
#define HTTP_POST_DATA BIT(5)

//! Set HTTP_V_1_1 to use HTTP/HTTPS version 1.1
#define HTTP_V_1_1 BIT(6)

//! Enable user defined HTTP/HTTPS content type in FLAGS
...
//! HTTP/HTTPS Server IP address.
#define HTTP_SERVER_IP_ADDRESS "192.168.1.177"

//! HTTP/HTTPS resource name
#define HTTP_URL "/index.html"

//! set HTTP/HTTPS hostname
#define HTTP_HOSTNAME "192.168.0.111"

//! set HTTP/HTTPS extended header
...
//! Enable this macro if Webpage content is returned as HTTP/HTTPS response
...
//! Max HTTP/HTTPS PUT buffer length
#define MAX_HTTP_CLIENT_PUT_BUFFER_LENGTH 900

//! HTTP/HTTPS response received
volatile int32_t rsp_received;

//! End of HTTP/HTTPS put file/content
volatile uint8_t end_of_file;

//! HTTP/HTTPS receive status
...
int32_t rsi_http_client_app()
{
  ...
  //! Create HTTPS client for PUT method
  status = rsi_http_client_put_create();
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nCreate HTTPS client Failed, Error Code : 0x%lX\r\n", status);
    return status;
  } else {
    LOG_PRINT("\r\nCreate HTTPS client Success\r\n");
  }

  //! Start HTTPS client PUT method for the given URL
  ...
  while (!end_of_file) {
    ...
    //! Send resource content to the HTTPS server for the given URL and total content length
...
//! Enable this macro if Webpage content is returned as HTTPS response
  ...
  //! send HTTPS get request for the given url
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nHTTPS Get request failed, Error Code : 0x%lX\r\n", status);
    return status;
  } else {
    LOG_PRINT("\r\nHTTPS Get request Success\r\n");
  }

  //! resetting the variables
  rsi_http_reset_handler();

  //! send HTTPS post request for the given url with given HTTPS data
  ...
  status = rsi_http_response_status((int32_t *)&http_recv_status);
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nHTTPS Post request failed, Error Code : 0x%lX\r\n", status);
    return status;
  } else {
    LOG_PRINT("\r\nHTTPS Post request Success\r\n");
#ifndef RSI_WITH_OS
...
//! HTTPS put response notify call back handler
void rsi_http_client_put_response_handler(uint16_t status,
                                          const uint8_t type,
                                          const uint8_t *buffer,
                                          uint16_t length,
...
//! HTTPS get/post response notify call back handler
#if RSI_HTTP_STATUS_INDICATION_EN
void rsi_http_response_handler(uint16_t status,
                               const uint8_t *buffer,
                               const uint16_t length,
...
#if RSI_HTTP_STATUS_INDICATION_EN
  if (count == 1) {
    LOG_PRINT("\r\nHTTPS response Handler ,Status code :%ld\r\n", status_code);
    count++;
  }
#endif
#ifdef RSI_WITH_OS
