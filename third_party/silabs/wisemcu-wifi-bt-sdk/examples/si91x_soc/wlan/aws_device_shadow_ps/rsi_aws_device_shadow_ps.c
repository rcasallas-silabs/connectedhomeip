...
#define RSI_M4_POWERSAVE_TASK_STACK_SIZE (512)
#define RSI_M4_POWERSAVE_TASK_PRIORITY   1
...
extern rsi_semaphore_handle_t m4_powersave_sem;
#endif
rsi_wlan_app_context_t rsi_wlan_app_context;

uint8_t global_buf[GLOBAL_BUFF_LEN], aws_temp_disp_count;
...
#if ENABLE_POWER_SAVE
          rsi_wlan_app_context.state = RSI_SLEEP_STATE;
          break; // shadow publish loop end
#endif
        }
#ifdef RSI_WITH_OS
        rsi_semaphore_post(&rsi_shadow_sem);
...
#ifdef RSI_WITH_OS
        rsi_semaphore_post(&m4_powersave_sem);
#else
        // configure powersave
        m4_powersave_app();
#endif
...
int main()
{
  ...
  rsi_task_handle_t application_handle = NULL;
#if ENABLE_POWER_SAVE
  rsi_task_handle_t m4_powersave_app_handle = NULL;
#endif
  ...
  rsi_task_create((rsi_task_function_t)(int32_t)application,
                  ...
                  &application_handle);
#if ENABLE_POWER_SAVE
  // Create m4_powersave_app task
  rsi_task_create((rsi_task_function_t)(int32_t)m4_powersave_app,
                  (uint8_t *)"m4_powersave_app_task",
                  RSI_M4_POWERSAVE_TASK_STACK_SIZE,
                  NULL,
                  RSI_M4_POWERSAVE_TASK_PRIORITY,
                  &m4_powersave_app_handle);
#endif
  // Start the scheduler
