/*******************************************************************************
* @file  rsi_tcp_tx_with_powersave_on.c
* @brief 
*******************************************************************************
* # License
 ...
 * consumption of Silabs device in Connected sleep mode having TCP server socket 
 * in open state.M4 wakes up with alarm configured time and sends packet to NWP, 
 * NWP wakes up with packet from M4 then process packet after that both will go to sleep.�
	=================================================================================*/
/**
 * Include files
 * */
...
#define SERVER_IP_ADDRESS 0x2E2BA8C0
...
void M4_sleep_wakeup()
{
  ...
  LOG_PRINT("\r\nTIME: %d:%d:%d:%d\n\r", readTime.Hour, readTime.Minute, readTime.Second, readTime.MilliSeconds);
  LOG_PRINT("\r\nDATE: %d/%d/%d\n\n\r", readTime.Day, readTime.Month, (readTime.Year + BASE_YEAR));
#endif
  ...
  while (1) {
    ...
    switch (rsi_wlan_app_cb.state) {
      case RSI_WLAN_INITIAL_STATE: {
        memset(send_buf, 0xFE, sizeof(send_buf));
      ...
      case RSI_WLAN_SOCKET_CREATE_STATE: {
        ...
        if (status != RSI_SUCCESS) {
          status = rsi_wlan_get_status();
          LOG_PRINT("\r\n Connect to server socket Failed 0x%X \r\n", status);
          do {
      ...
      case RSI_WLAN_SEND_SOCKET_DATA_STATE: {
        ...
        while (packet_count < NUMBER_OF_PACKETS) {
          ...
          if (status < 0) {
            status = rsi_wlan_get_status();
            LOG_PRINT("\r\n Sending Data to Server Socket Fail \r\n");
...
int main()
{
  ...
  LOG_PRINT("\r\nCCP-Powersave TCP_Tx_With_Powersave Application Demo\r\n");
