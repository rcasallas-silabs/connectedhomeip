/*******************************************************************************
* @file  rsi_periodic_tx_deepsleep.c
* @brief 
*******************************************************************************
* # License
 ...
 * @section Description : 
 * TCP Tx with powersave on periodic wakeup test application demonstrates power 
 * consumption of Silabs device in deep sleep mode having TCP server socket in open 
 * state.M4 wakes up with alarm configured time and sends packet to NWP, NWP wakes up 
 * with packet from M4 then process packet after that both will go to sleep.�
=================================================================================*/
...
//! Enable this for 1.4Soc based EVKs
...
#define NUMBER_OF_PACKETS 44 //64 //2872 // 512kbps - ( (512*1024*8)/1460 )
...
#define BUF_SIZE 1460 //1400 //1024 //1400 //1460 //512
uint8_t send_buf[BUF_SIZE] = { 0xFF };

volatile uint8_t gotosleep = 0, alarm_config = 0, ta_powersaved = 0;
...
#define SLEEP_ALARM_PERIODIC_TIME  300 /*<! periodic alarm configuration in SEC */
#define ACTIVE_ALARM_PERIODIC_TIME 300 /*<! periodic alarm configuration in SEC */
...
typedef enum rsi_wlan_app_state_e {
  ...
  RSI_WLAN_SOCKET_CREATE_STATE    = 5,
  RSI_WLAN_SOCKET_CONNECTED_STATE = 6,
  RSI_WLAN_SEND_SOCKET_DATA_STATE = 7,
  RSI_WLAN_TRIGGER_SLEEP_STATE    = 8
...
void IRQ029_Handler()
{
  ...
  RSI_PS_ClrWkpSources(SEC_BASED_WAKEUP);
}
  ...
  while (1) {
    ...
    switch (rsi_wlan_app_cb.state) {
      ...
      case RSI_WLAN_IPCONFIG_DONE_STATE: {
        rsi_wlan_app_cb.state = RSI_WLAN_SOCKET_CREATE_STATE;
      } break;
      case RSI_WLAN_SOCKET_CREATE_STATE: {
        if (ta_powersaved) {
      ...
      case RSI_WLAN_SOCKET_CONNECTED_STATE: {
        ...
        rsi_wlan_app_cb.state = RSI_WLAN_SEND_SOCKET_DATA_STATE;
      } break;
      case RSI_WLAN_SEND_SOCKET_DATA_STATE: {
        ...
        //! instead of delay, going into sleep
        //while(((rsi_hal_gettickcount() - start) < 1000));
        ...
        if (packet_count == NUMBER_OF_PACKETS) {
          ...
          if (gotosleep == 1) {
            rsi_wlan_app_cb.state = RSI_WLAN_TRIGGER_SLEEP_STATE;
          } else {
            continue;
          }
        }
      } break;
      case RSI_WLAN_TRIGGER_SLEEP_STATE: {
        do {
          status = rsi_shutdown(client_socket, 0);
        } while (status != RSI_SUCCESS);

        //! wait for TA ready
        while ((P2P_STATUS_REG & TA_wakeup_M4)) {
          rsi_wireless_driver_task();
        }

        status = rsi_wlan_disconnect();
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\r\n Powersave Disable Failed ,Error code :0x%x \r\n", status);
          while (1)
            ;
        }
        ...
        /* update sleep time   */
        RSI_Set_Alarm_Intr_Time(SLEEP_ALARM_PERIODIC_TIME);
...
int main()
{
  ...
  LOG_PRINT("\r\nCCP-Powersave Periodic_Tx_Deepsleep Application Demo\r\n");
 ...
 * @brief        This function used to update the alarm interval   
 *              
 * @param[in]    interval  : Alarm  Timer interrut time
 * @param[out]   None
 *
 */
void RSI_Set_Alarm_Intr_Time(uint16_t interval)
 ...
 * @brief        This function used to Intialize  Alarm and RTC block   
 *              
 * @param[in]    interval  : Alarm  Timer interrut time
 * @param[out]   None
 *
...
void InitM4AlarmConfig(void)
{
  ...
  /*Enables RTC NVIC */
  NVIC_EnableIRQ(MCU_CAL_RTC_IRQn);
