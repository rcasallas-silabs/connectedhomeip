...
/* Private Variable ---------------------------------------------------------*/
volatile uint8_t reconfig = 0;
volatile uint32_t intr_cnt;

/* Standard sin wave samples */
// f~=10K (11.7K), fs=4M
int16_t input_sample_tone10khz[BUFFER_SIZE] = {
...
extern void rsi_delay_ms(uint32_t val);
...
void soc_pll_config(void)
{
  ...
  RSI_ULPSS_UlpProcClkConfig(ULPCLK, ULP_PROC_SOC_CLK, 0, &rsi_delay_ms);
}
#endif

/**
 * @brief  Event call back function..
 * @param  event       : DAC_UDMA_PING_PONG_CONFIG =>Reconfigure the external DMA ping or pong descriptor for DAC.
 * @retval None
 */
void dac_callback(uint8_t event)
{
  if (event == DAC_UDMA_PING_PONG_CONFIG) {
    reconfig = 1;
    intr_cnt++;
  }
}
...
int main(void)
{
  /* At this stage the MICROCONTROLLER clock setting is already configured,
	 ...
	 */
  volatile uint32_t achieved_sample_freq = 0;
...
#if CORE_CLOCK_180MHZ
  /* Switch M4 Processor clock to 180 MHZ */
  soc_pll_config();
#endif
  ...
  achieved_sample_freq = DAC_Init(FIFO_MODE_EN, SAMPLING_RATE, dac_callback);

  DEBUGOUT("\r\nConfigured sampling rate to DAC\r\n");
  ...
  status = RSI_AUX_RefVoltageConfig(vref, battery_status);
  if (status != RSI_OK) {
    /* Failed to Set Reference Voltage to DAC */
    DEBUGOUT("\r\nFailed to Set Reference Voltage to DAC\r\n");
    return status;
  } else {
    /* Set Reference Voltage to DAC */
    DEBUGOUT("\r\nSet Reference Voltage to DAC\r\n");
  }

  status = DAC_WriteData(FIFO_MODE_EN, input_sample_tone10khz, BUFFER_SIZE);
  if (status != RSI_OK) {
    /* Failed to write digital samples data to DAC */
    DEBUGOUT("\r\nFailed to write digital samples data to DAC\r\n");
    return status;
  } else {
    /* Write digital samples data to DAC */
    DEBUGOUT("\r\nWrite digital samples data to DAC\r\n");
  }

  /* Start DAC */
  DEBUGOUT("\r\nStart DAC\r\n");
  DAC_Start(FIFO_MODE_EN);

  while (1) {
    /* Wait uDMA done interrupt */
    while (!reconfig)
      ;
    reconfig = 0;
    DAC_PingPongReconfig(input_sample_tone10khz, BUFFER_SIZE);
  }
}
