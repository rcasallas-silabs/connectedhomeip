...
rsi_semaphore_handle_t rsi_mqtt_sem, pwm_thread_sem, i2c_thread_sem, publish_msg_sem;
#endif
  ...
  while (1) {
    ...
    switch (rsi_wlan_app_cb.state) {
      ...
      case RSI_WLAN_MQTT_SUBSCRIBE_STATE: {
        ...
        rsi_semaphore_wait(&publish_msg_sem, 0);
#endif
      } break;

      case RSI_WLAN_MQTT_PUBLISH_STATE: {
        LOG_PRINT("\n AWS IOT MQTT Publish... \n");
        strncpy(publish_message_mq, rd_buf, 16);
        ...
        for (i = 0; i < AWS_IOT_MQTT_RX_BUF_LEN; i++) {
          if (client.clientData.readBuf[i] == 'A' && client.clientData.readBuf[i + 1] == 'W'
              && client.clientData.readBuf[i + 2] == 'S') {

#ifdef RSI_M4_INTERFACE
            LOG_PRINT("\n LED clear \n");
            RSI_Board_LED_Set(0, false);
            LOG_PRINT("\n LED set as AWS data is received \n");
