...
int main(void)
{
  ...
  if (status != RSI_OK) {
   ...
   status = ROMAPI_M4SS_CLK_API->clk_qspi_clk_config(M4CLK, QSPI_INTFPLLCLK, 0, 0, 2);
  if (status != RSI_OK) {
  ...
  status = SPIdrv->Transfer(testdata_out, testdata_in, sizeof(testdata_out));
  if (status != ARM_DRIVER_OK) {
