...
def initWorkspaceAndScm()
{
    ...
    dir(buildOverlayDir+createWorkspaceOverlay.overlayMatterPath)
    {
        ...
        /*TODO: Below line to be uncommented once Wifi-SDK Hash commit matches with Github & Stash*/
                    ...
                    withDockerContainer(image: image)
                    {
                        withCredentials([usernamePassword(credentialsId: 'svc_gsdk', passwordVariable: 'SL_PASSWORD', usernameVariable: 'SL_USERNAME')])
                        {
                                ...
                                sh '''
                                    ...
                                    zip -r "${ota_file}" "scripts/tools/silabs" "src/app/ota_image_tool.py" "src/controller/python/chip/tlv/__init__.py" "src/controller/python/chip/tlv/tlvlist.py" -x "*.md" "provision/samples/light/3"
